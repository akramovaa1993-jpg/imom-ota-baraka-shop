...
<style>.seo-cart-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.48);z-index:9998}.seo-cart-overlay.open{display:block}.seo-cart{position:fixed;top:0;right:-460px;width:min(430px,92vw);height:100vh;background:#fff;z-index:9999;box-shadow:-12px 0 35px rgba(0,0,0,.18);transition:right .25s ease;display:flex;flex-direction:column;color:#17351f}.seo-cart.open{right:0}.seo-cart-head{padding:22px;border-bottom:1px solid #e3ebe5;display:flex;justify-content:space-between;align-items:center}.seo-cart-head h2{margin:0}.seo-cart-close{border:0;background:none;font-size:28px;cursor:pointer}.seo-cart-body{padding:18px;overflow:auto;flex:1}.seo-cart-row{display:grid;grid-template-columns:58px 1fr auto;gap:12px;padding:14px 0;border-bottom:1px solid #e7eee9}.seo-cart-row img{width:58px;height:58px;object-fit:contain;border-radius:10px;background:#f5f7f5}.seo-cart-name{font-weight:700;font-size:14px}.seo-cart-price{font-weight:800;white-space:nowrap}.seo-cart-qty{display:flex;align-items:center;gap:8px;margin-top:8px}.seo-cart-qty button{width:30px;height:30px;border:0;border-radius:8px;background:#edf5ee;cursor:pointer}.seo-cart-remove{border:0;background:none;color:#b33;cursor:pointer;font-size:18px}.seo-cart-foot{padding:18px;border-top:1px solid #e3ebe5}.seo-cart-total{display:flex;justify-content:space-between;font-size:19px;font-weight:800;margin-bottom:14px}.seo-cart-order{display:block;text-align:center;background:#1f6b35;color:#fff;text-decoration:none;border-radius:12px;padding:15px;font-weight:800}.seo-cart-empty{text-align:center;color:#718076;padding:50px 10px}</style></head><body><main class="wrap"><header class="top"><a href="/"><img src="/logo.png" alt="IMOM OTA BARAKA"></a><div><a href="/"><b>ZARBULOQ.UZ</b></a><div>${isRu?'Интернет-магазин IMOM OTA BARAKA':'IMOM OTA BARAKA internet do‘koni'}</div></div></header><section class="card"><div class="media"><img src="${htmlEsc(img)}" alt="${htmlEsc(name)}" loading="eager"></div><div><div class="cat">${htmlEsc(cat)}</div><h1>${htmlEsc(name)}</h1><div class="ratingline"><span class="stars">${seoSummary.count?'★'.repeat(Math.round(seoSummary.average))+'☆'.repeat(5-Math.round(seoSummary.average)):'☆☆☆☆☆'}</span><b>${seoSummary.count?seoSummary.average.toFixed(1):'—'}</b><span>(${seoSummary.count} ${isRu?'отзывов':'ta baho'})</span></div><p class="desc">${htmlEsc(desc)}</p><div class="price">${oldPrice}${price.toLocaleString('ru-RU')} ${isRu?'сум':'so‘m'}</div><div class="stock">${htmlEsc(stockText)}</div><div class="actions">${stock>0?`<button class="btn primary" onclick="addToCartAndOpen()">${isRu?'В корзину':'Savatga qo‘shish'}</button>`:''}<a class="btn secondary" href="/#products">${isRu?'Все товары':'Barcha mahsulotlar'}</a></div></div></section><section class="reviews"><h2>${isRu?'Оценки и отзывы':'Baholar va izohlar'} (${seoSummary.count})</h2><div class="review-grid"><div class="summary-box"><div>${isRu?'Общая оценка':'Umumiy baho'}</div><div class="avg">${seoSummary.count?seoSummary.average.toFixed(1):'—'}</div><div class="stars">${seoSummary.count?'★'.repeat(Math.round(seoSummary.average))+'☆'.repeat(5-Math.round(seoSummary.average)):'☆☆☆☆☆'}</div><p>${seoSummary.count} ${isRu?'оценок':'ta baho asosida'}</p></div><form class="review-form" onsubmit="submitReview(event)"><h3>${isRu?'Оставьте свой отзыв':'O‘z fikringizni qoldiring'}</h3><div class="pickstars" id="pickStars">${[1,2,3,4,5].map(n=>`<button type="button" onclick="pickRating(${n})">★</button>`).join('')}</div><input type="hidden" id="ratingValue" value="0"><input id="reviewName" placeholder="${isRu?'Ваше имя':'Ismingiz'}" required><input id="reviewPhone" placeholder="+998 90 123 45 67" required><textarea id="reviewComment" maxlength="500" placeholder="${isRu?'Ваш отзыв':'Mahsulot haqida fikringizni yozing...'}" required></textarea><button class="btn primary" type="submit">${isRu?'Отправить отзыв':'Izoh qoldirish'}</button><div id="reviewStatus"></div></form></div><div id="reviewRows">${seoReviews.slice(0,50).map(r=>`<article class="review-row"><div class="review-meta"><b>${htmlEsc(r.name||'Foydalanuvchi')}</b><span class="stars">${'★'.repeat(Math.max(1,Math.min(5,Number(r.rating)||1)))+'☆'.repeat(5-Math.max(1,Math.min(5,Number(r.rating)||1)))}</span><small>${htmlEsc(String(r.updatedAt||r.createdAt||'').slice(0,10))}</small></div><p>${htmlEsc(r.comment||'')}</p>${r.adminReply?`<div class="admin-reply"><b>${isRu?'Ответ администратора':'Admin javobi'}</b>${htmlEsc(r.adminReply)}</div>`:''}</article>`).join('')||`<p>${isRu?'Отзывов пока нет.':'Hali izoh yo‘q.'}</p>`}</div></section></main><div id="seoCartOverlay" class="seo-cart-overlay" onclick="closeSeoCart()"></div><aside id="seoCart" class="seo-cart"><div class="seo-cart-head"><div><h2>${isRu?'Корзина':'Savat'}</h2><small id="seoCartCount"></small></div><button class="seo-cart-close" onclick="closeSeoCart()">×</button></div><div id="seoCartBody" class="seo-cart-body"></div><div class="seo-cart-foot"><div class="seo-cart-total"><span>${isRu?'Итого':'Jami'}</span><span id="seoCartTotal">0 so‘m</span></div><a class="seo-cart-order" href="/#cart">${isRu?'Оформить заказ':'Buyurtma berish'}</a></div></aside><script>let chosenRating=0;const seoProduct={id:${JSON.stringify(Number(p.id))},name:${JSON.stringify(name)},price:${JSON.stringify(price)},image:${JSON.stringify(img)},stock:${JSON.stringify(stock)}};function pickRating(n){chosenRating=n;document.getElementById('ratingValue').value=n;[...document.querySelectorAll('#pickStars button')].forEach((b,i)=>b.classList.toggle('on',i<n))}async function submitReview(e){e.preventDefault();const st=document.getElementById('reviewStatus');if(chosenRating<1){st.textContent='${isRu?'Выберите оценку':'Baho tanlang'}';return}const body={name:document.getElementById('reviewName').value.trim(),phone:document.getElementById('reviewPhone').value.trim(),rating:chosenRating,comment:document.getElementById('reviewComment').value.trim()};st.textContent='${isRu?'Отправляем...':'Yuborilmoqda...'}';try{const r=await fetch('/api/products/${encodeURIComponent(String(p.id))}/reviews',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok)throw new Error(d.error||'Xatolik');location.reload()}catch(err){st.textContent=err.message}}function getSeoCart(){try{return JSON.parse(localStorage.getItem('iob_cart_v13')||'[]')}catch(e){return[]}}function saveSeoCart(c){localStorage.setItem('iob_cart_v13',JSON.stringify(c))}function addToCartAndOpen(){try{let cart=getSeoCart();let x=cart.find(a=>Number(a.id)===seoProduct.id);if(x){if(Number(x.qty)>=seoProduct.stock){alert('${isRu?'Максимальное количество уже в корзине.':'Ombordagi maksimal miqdor savatda.'}');openSeoCart();return}x.qty=Number(x.qty||0)+1}else{cart.push({id:seoProduct.id,qty:1})}saveSeoCart(cart);openSeoCart()}catch(e){console.error(e)}}function openSeoCart(){renderSeoCart();document.getElementById('seoCart').classList.add('open');document.getElementById('seoCartOverlay').classList.add('open');document.body.style.overflow='hidden'}function closeSeoCart(){document.getElementById('seoCart').classList.remove('open');document.getElementById('seoCartOverlay').classList.remove('open');document.body.style.overflow=''}function changeSeoQty(id,d){let c=getSeoCart(),x=c.find(a=>Number(a.id)===Number(id));if(!x)return;x.qty=Math.max(1,Number(x.qty||1)+d);if(Number(id)===seoProduct.id&&x.qty>seoProduct.stock)x.qty=seoProduct.stock;saveSeoCart(c);renderSeoCart()}function removeSeoCart(id){saveSeoCart(getSeoCart().filter(a=>Number(a.id)!==Number(id)));renderSeoCart()}function renderSeoCart(){const c=getSeoCart(),body=document.getElementById('seoCartBody');document.getElementById('seoCartCount').textContent=c.length+' ${isRu?'товар(а)':'ta mahsulot'}';if(!c.length){body.innerHTML='<div class="seo-cart-empty">${isRu?'Корзина пуста':'Savat bo‘sh'}</div>';document.getElementById('seoCartTotal').textContent='0 so‘m';return}let total=0;body.innerHTML=c.map(x=>{const same=Number(x.id)===seoProduct.id,n=same?seoProduct.name:'${isRu?'Товар':'Mahsulot'} #'+x.id,pr=same?seoProduct.price:0,im=same?seoProduct.image:'/logo.png',q=Number(x.qty||1);total+=pr*q;return '<div class="seo-cart-row"><img src="'+im+'" alt=""><div><div class="seo-cart-name">'+n+'</div><div class="seo-cart-qty"><button onclick="changeSeoQty('+x.id+',-1)">−</button><b>'+q+'</b><button onclick="changeSeoQty('+x.id+',1)">+</button></div></div><div><div class="seo-cart-price">'+(pr*q).toLocaleString('ru-RU')+' so‘m</div><button class="seo-cart-remove" onclick="removeSeoCart('+x.id+')">×</button></div></div>'}).join('');document.getElementById('seoCartTotal').textContent=total.toLocaleString('ru-RU')+' so‘m'}</script></body></html>`);
}
