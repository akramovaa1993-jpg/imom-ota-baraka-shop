...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.11.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,dataFile:DB_FILE}));
...
app.listen(PORT,()=>{console.log(`IMOM OTA BARAKA v13.11 ZARBULOQ / zarbuloq.uz: http://localhost:${PORT}`);console.log(`Telegram CHAT_ID: ${CHAT_ID?'configured':'MISSING'}`);console.log(`Telegram BOT_TOKEN: ${BOT_TOKEN?'configured':'MISSING'}`);console.log(`Online admin: /admin.html | DATA_DIR=${DATA_DIR}`);pollTelegram();});
