...
const defaultSettings={
 ...
 homePromos:{
  ...
  right:{title:{uz:'Sifat. Ishonch. Baraka!',ru:'Качество. Доверие. Баракат!'},text:{uz:'IMOM OTA BARAKA — har doim siz bilan.',ru:'IMOM OTA BARAKA — всегда рядом с вами.'},button:{uz:'Batafsil',ru:'Подробнее'},image:''}
 },
 benefits:[
...
function normalizeDb(db){
 const merged={...initialDb(),...(db||{}),settings:{...defaultSettings,...(db?.settings||{}),delivery:{...defaultSettings.delivery,...(db?.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db?.settings?.seo||{})},company:{...defaultSettings.company,...(db?.settings?.company||{})},map:{...defaultSettings.map,...(db?.settings?.map||{})},footer:{...defaultSettings.footer,...(db?.settings?.footer||{})},ui:{...defaultSettings.ui,...(db?.settings?.ui||{})},testMode:{...defaultSettings.testMode,...(db?.settings?.testMode||{})},homePromos:{left:{...defaultSettings.homePromos.left,...(db?.settings?.homePromos?.left||{})},center:{...defaultSettings.homePromos.center,...(db?.settings?.homePromos?.center||{})},right:{...defaultSettings.homePromos.right,...(db?.settings?.homePromos?.right||{})}},benefits:Array.isArray(db?.settings?.benefits)?db.settings.benefits:defaultSettings.benefits},chats:Array.isArray(db?.chats)?db.chats:[],promos:Array.isArray(db?.promos)?db.promos:defaultPromos,audit:Array.isArray(db?.audit)?db.audit:[]};
...
app.get('/api/admin/inventory-report.xls',requireAdmin,(req,res)=>{
 const db=readDb(),month=/^\d{4}-\d{2}$/.test(String(req.query.month||''))?String(req.query.month):new Date().toISOString().slice(0,7),start=month+'-01',end=new Date(Number(month.slice(0,4)),Number(month.slice(5,7)),0).toISOString().slice(0,10),orders=db.orders||[];
 const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const qtyFor=(pid,statuses,from,to)=>orders.filter(o=>statuses.includes(o.status)&&(!from||String(o.createdAt).slice(0,10)>=from)&&(!to||String(o.createdAt).slice(0,10)<=to)).reduce((sum,o)=>sum+(o.items||[]).filter(i=>Number(i.id)===Number(pid)).reduce((a,i)=>a+Number(i.qty||0),0),0);
 const rows=(db.products||[]).map(p=>{const created=String(p.createdAt||'').slice(0,10),received=Number(p.receivedQty??p.stock??0),receivedMonth=created>=start&&created<=end?received:0,receivedBefore=created&&created<start?received:0,soldBefore=qtyFor(p.id,['delivery','done'],'',new Date(new Date(start).getTime()-86400000).toISOString().slice(0,10)),opening=Math.max(0,receivedBefore-soldBefore),sold=qtyFor(p.id,['delivery','done'],start,end),applications=qtyFor(p.id,['new'],start,end);return `<tr><td>${esc(p.name?.uz)}</td><td>${esc(p.name?.ru)}</td><td>${esc(p.unit?.uz||'dona')}</td><td>${Number(p.cost||0)}</td><td>${Number(p.price||0)}</td><td>${opening}</td><td>${receivedMonth}</td><td>${sold}</td><td>${applications}</td><td>${Number(p.stock||0)}</td><td>${esc(created||'—')}</td></tr>`}).join('');
 const html=`<html><head><meta charset="UTF-8"></head><body><h2>IMOM OTA BARAKA — Ombor hisoboti ${esc(month)}</h2><table border="1"><tr><th>Mahsulot nomi</th><th>Название товара</th><th>Birlik</th><th>Tannarxi</th><th>Sotuv narxi</th><th>Oy boshiga ostatka</th><th>Qabul qilingan</th><th>Sotilgan (Yetkazilmoqda + Yakunlandi)</th><th>Zayavka (Yangi)</th><th>Joriy qoldiq</th><th>Qabul sanasi</th></tr>${rows}</table></body></html>`;res.setHeader('Content-Type','application/vnd.ms-excel; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="zarbuloq-ombor-${month}.xls"`);res.send('\ufeff'+html);
});
