...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.25.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,storage:pool?'postgresql':'local-json',persistent:Boolean(pool),dataFile:DB_FILE}));
...
async function start(){
 try{
  await initStorage();
  app.listen(PORT,()=>{
   console.log(`IMOM OTA BARAKA v13.25 ERP PRO BALANS + DASHBOARD ANALYTICS + PRO PRAYS + OMBOR + PRODUCT SEO + LIVE CHAT ZARBULOQ / zarbuloq.uz: http://localhost:${PORT}`);
