...
function initialDb(){return {products:defaultProducts,categories:defaultCategories,settings:defaultSettings,logo:'',orders:[],productRequests:[],chats:[],promos:defaultPromos,audit:[],inventoryReceipts:[],financeCompanies:[],financePurchases:[],financeCompanyPayments:[],financeEmployees:[],financePayroll:[],financeExpenses:[],financeTaxPayments:[],financeSettings:{turnoverTaxRate:4,payrollTaxRate:0,landTaxMonthly:0,propertyTaxRate:0,propertyTaxBase:0,otherTaxRate:0,otherTaxBase:'revenue',otherTaxMonthly:0,cashOpening:0,bankOpening:0,defaultSalesAccount:'bank',defaultMarkupRate:30}};}
function normalizeDb(db){
 const merged={...initialDb(),...(db||{}),settings:{...defaultSettings,...(db?.settings||{}),delivery:{...defaultSettings.delivery,...(db?.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db?.settings?.seo||{})},company:{...defaultSettings.company,...(db?.settings?.company||{})},map:{...defaultSettings.map,...(db?.settings?.map||{})},footer:{...defaultSettings.footer,...(db?.settings?.footer||{})},ui:{...defaultSettings.ui,...(db?.settings?.ui||{})},testMode:{...defaultSettings.testMode,...(db?.settings?.testMode||{})}},chats:Array.isArray(db?.chats)?db.chats:[],promos:Array.isArray(db?.promos)?db.promos:defaultPromos,audit:Array.isArray(db?.audit)?db.audit:[],inventoryReceipts:Array.isArray(db?.inventoryReceipts)?db.inventoryReceipts:[],financeCompanies:Array.isArray(db?.financeCompanies)?db.financeCompanies:[],financePurchases:Array.isArray(db?.financePurchases)?db.financePurchases:[],financeCompanyPayments:Array.isArray(db?.financeCompanyPayments)?db.financeCompanyPayments:[],financeEmployees:Array.isArray(db?.financeEmployees)?db.financeEmployees:[],financePayroll:Array.isArray(db?.financePayroll)?db.financePayroll:[],financeExpenses:Array.isArray(db?.financeExpenses)?db.financeExpenses:[],financeTaxPayments:Array.isArray(db?.financeTaxPayments)?db.financeTaxPayments:[],financeSettings:{...initialDb().financeSettings,...(db?.financeSettings||{})}};
...
function financeSummary(db,opts={}){
 ...
 const payrollTax=payrollGross*finNum(fsx.payrollTaxRate)/100;
 ...
 const taxAccrued=turnoverTax+payrollTax+landTax+propertyTax+otherTax;
 ...
 return {revenue,cogs,grossProfit,purchaseTotal,supplierPaid,payrollGross,payrollPaid,otherExpenses,turnoverTax,payrollTax,landTax,propertyTax,otherTax,taxAccrued,taxPaid,taxDebt:Math.max(0,taxAccrued-taxPaid),netProfit,orders:done.length};
}
function financeCurrentBalance(db){
 ...
 return {inventoryValue,supplierDebt,payrollDebt,taxDebt,cash,bank,liquid:cash+bank,assets:cash+bank+inventoryValue,liabilities:supplierDebt+payrollDebt+taxDebt,equity:(cash+bank+inventoryValue)-(supplierDebt+payrollDebt+taxDebt)};
}
...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.23.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,storage:pool?'postgresql':'local-json',persistent:Boolean(pool),dataFile:DB_FILE}));
...
app.get('/api/admin/finance',requireAdmin,(req,res)=>{
 const db=readDb(),period=clean(req.query.period,20)||'monthly',value=clean(req.query.value,20),year=clean(req.query.year,4)||String(new Date().getFullYear());
 res.json({ok:true,settings:db.financeSettings||{},companies:financeCompanyBalances(db),purchases:(db.financePurchases||[]).slice().reverse().slice(0,1000),companyPayments:(db.financeCompanyPayments||[]).slice().reverse().slice(0,1000),employees:db.financeEmployees||[],payroll:(db.financePayroll||[]).slice().reverse().slice(0,1000),expenses:(db.financeExpenses||[]).slice().reverse().slice(0,1500),taxPayments:(db.financeTaxPayments||[]).slice().reverse().slice(0,1500),summary:financeSummary(db,{period,value}),balance:financeCurrentBalance(db),series:financeSeries(db,year)});
});
app.put('/api/admin/finance/settings',requireAdmin,async(req,res)=>{const db=readDb(),b=req.body||{},f=db.financeSettings||{};for(const k of ['turnoverTaxRate','payrollTaxRate','landTaxMonthly','propertyTaxRate','propertyTaxBase','otherTaxRate','otherTaxMonthly','cashOpening','bankOpening','defaultMarkupRate'])f[k]=finNum(b[k]);f.otherTaxBase=['revenue','payroll','fixed'].includes(b.otherTaxBase)?b.otherTaxBase:'revenue';f.defaultSalesAccount=b.defaultSalesAccount==='cash'?'cash':'bank';db.financeSettings=f;audit(db,req.adminUser,'Finans sozlamalari','Soliq va balans sozlamalari yangilandi');await writeDb(db);res.json({ok:true,settings:f});});
...
app.post('/api/admin/finance/purchases',requireAdmin,async(req,res)=>{try{const db=readDb(),b=req.body||{},company=(db.financeCompanies||[]).find(x=>String(x.id)===String(b.companyId)),product=(db.products||[]).find(x=>Number(x.id)===Number(b.productId)),qty=finNum(b.qty),unitCost=finNum(b.unitCost);if(!company||!product||qty<=0||unitCost<=0)return res.status(400).json({error:'Firma, mahsulot, miqdor va xarid narxini kiriting'});const transport=finNum(b.transport),extra=finNum(b.extra),total=qty*unitCost+transport+extra,unitLanded=total/qty,oldQty=finNum(product.stock),oldCost=finNum(product.cost),newQty=oldQty+qty,weighted=newQty>0?((oldQty*oldCost)+total)/newQty:unitLanded,markup=finNum(b.markupRate||db.financeSettings?.defaultMarkupRate),suggested=Math.round(unitLanded*(1+markup/100));product.stock=newQty;product.cost=Math.round(weighted);if(Boolean(b.updateSalePrice))product.price=suggested;product.lastReceivedAt=new Date().toISOString();const date=clean(b.date,10)||new Date().toISOString().slice(0,10),x={id:'PUR-'+Date.now(),companyId:company.id,productId:product.id,qty,unitCost,transport,extra,total,unitLanded:Math.round(unitLanded),markupRate:markup,suggestedSalePrice:suggested,date,invoice:clean(b.invoice,80),note:clean(b.note,300),createdAt:new Date().toISOString(),actor:req.adminUser};db.financePurchases.push(x);db.inventoryReceipts.push({receiptId:`RCV-${Date.now()}-${crypto.randomInt(10,99)}`,productId:product.id,qty,createdAt:new Date(date+'T12:00:00').toISOString(),actor:req.adminUser,source:'finance-purchase',purchaseId:x.id});const paid=finNum(b.paidAmount);if(paid>0)db.financeCompanyPayments.push({id:'PAY-'+Date.now()+'-P',companyId:company.id,amount:Math.min(paid,total),date,account:b.account==='cash'?'cash':'bank',note:`Xarid ${x.id} uchun boshlang‘ich to‘lov`,purchaseId:x.id,createdAt:new Date().toISOString(),actor:req.adminUser});audit(db,req.adminUser,'Finans xaridi',`${company.name} • ${product.name?.uz||''} × ${qty} • ${money(total)}`);await writeDb(db);res.json({ok:true,purchase:x,stock:product.stock,cost:product.cost,suggestedSalePrice:suggested});}catch(e){console.error(e);res.status(500).json({error:'Xaridni saqlab bo‘lmadi'})}});
...
async function start(){
 try{
  await initStorage();
  app.listen(PORT,()=>{
   console.log(`IMOM OTA BARAKA v13.23 FINANS + DASHBOARD ANALYTICS + PRO PRAYS + OMBOR + PRODUCT SEO + LIVE CHAT ZARBULOQ / zarbuloq.uz: http://localhost:${PORT}`);
