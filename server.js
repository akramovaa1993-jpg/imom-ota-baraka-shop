...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.24.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,storage:pool?'postgresql':'local-json',persistent:Boolean(pool),dataFile:DB_FILE}));
...
app.get('/api/admin/finance',requireAdmin,(req,res)=>{
 ...
 res.json({ok:true,settings:db.financeSettings||{},companies:financeCompanyBalances(db),purchases:(db.financePurchases||[]).slice().reverse().slice(0,1000),companyPayments:(db.financeCompanyPayments||[]).slice().reverse().slice(0,1000),employees:db.financeEmployees||[],payroll:(db.financePayroll||[]).slice().reverse().slice(0,1000),expenses:(db.financeExpenses||[]).slice().reverse().slice(0,1500),taxPayments:(db.financeTaxPayments||[]).slice().reverse().slice(0,1500),sales,products:products.map(p=>({id:Number(p.id),name:p.name?.uz||'',nameRu:p.name?.ru||'',stock:finNum(p.stock),cost:finNum(p.cost),price:finNum(p.price),unit:p.unit?.uz||'dona'})),summary:financeSummary(db,{period,value}),balance:financeCurrentBalance(db),series:financeSeries(db,year)});
});
...
app.post('/api/admin/finance/purchases',requireAdmin,async(req,res)=>{try{const db=readDb(),b=req.body||{},company=(db.financeCompanies||[]).find(x=>String(x.id)===String(b.companyId)),product=(db.products||[]).find(x=>Number(x.id)===Number(b.productId)),qty=finNum(b.qty),unitCost=finNum(b.unitCost);if(!company||!product||qty<=0||unitCost<=0)return res.status(400).json({error:'Firma, mahsulot, miqdor va xarid narxini kiriting'});const transport=finNum(b.transport),extra=finNum(b.extra),total=qty*unitCost+transport+extra,unitLanded=total/qty,oldQty=finNum(product.stock),oldCost=finNum(product.cost),newQty=oldQty+qty,weighted=newQty>0?((oldQty*oldCost)+total)/newQty:unitLanded,markup=finNum(b.markupRate||db.financeSettings?.defaultMarkupRate),suggested=Math.round(unitLanded*(1+markup/100)),manualSalePrice=finNum(b.salePrice),appliedSalePrice=manualSalePrice>0?manualSalePrice:suggested;product.stock=newQty;product.cost=Math.round(weighted);product.price=appliedSalePrice;product.lastReceivedAt=new Date().toISOString();const date=clean(b.date,10)||new Date().toISOString().slice(0,10),x={id:'PUR-'+Date.now(),companyId:company.id,productId:product.id,qty,unitCost,transport,extra,total,unitLanded:Math.round(unitLanded),markupRate:markup,suggestedSalePrice:suggested,salePrice:appliedSalePrice,date,invoice:clean(b.invoice,80),note:clean(b.note,300),createdAt:new Date().toISOString(),actor:req.adminUser};db.financePurchases.push(x);db.inventoryReceipts.push({receiptId:`RCV-${Date.now()}-${crypto.randomInt(10,99)}`,productId:product.id,qty,createdAt:new Date(date+'T12:00:00').toISOString(),actor:req.adminUser,source:'finance-purchase',purchaseId:x.id});const paid=finNum(b.paidAmount);if(paid>0)db.financeCompanyPayments.push({id:'PAY-'+Date.now()+'-P',companyId:company.id,amount:Math.min(paid,total),date,account:b.account==='cash'?'cash':'bank',note:`Xarid ${x.id} uchun boshlang‘ich to‘lov`,purchaseId:x.id,createdAt:new Date().toISOString(),actor:req.adminUser});audit(db,req.adminUser,'Finans prixod',`${company.name} • ${product.name?.uz||''} × ${qty} • ${money(total)} • sotuv ${money(appliedSalePrice)}`);await writeDb(db);res.json({ok:true,purchase:x,stock:product.stock,cost:product.cost,suggestedSalePrice:suggested,salePrice:appliedSalePrice});}catch(e){console.error(e);res.status(500).json({error:'Xaridni saqlab bo‘lmadi'})}});
...
app.delete('/api/admin/finance/employees/:id',requireAdmin,async(req,res)=>{const db=readDb(),idx=(db.financeEmployees||[]).findIndex(e=>String(e.id)===String(req.params.id));if(idx<0)return res.status(404).json({error:'Ishchi topilmadi'});const x=db.financeEmployees[idx],hasHistory=(db.financePayroll||[]).some(r=>String(r.employeeId)===String(x.id));if(hasHistory)return res.status(409).json({error:'Bu ishchida oylik tarixi bor. Hisobotlar buzilmasligi uchun butunlay o‘chirib bo‘lmaydi; “Ishdan bo‘shatish” orqali faolsizlantiring.'});db.financeEmployees.splice(idx,1);audit(db,req.adminUser,'Ishchi o‘chirildi',x.name);await writeDb(db);res.json({ok:true});});
...
async function start(){
 try{
  await initStorage();
  app.listen(PORT,()=>{
   console.log(`IMOM OTA BARAKA v13.24 ERP FINANS + DASHBOARD ANALYTICS + PRO PRAYS + OMBOR + PRODUCT SEO + LIVE CHAT ZARBULOQ / zarbuloq.uz: http://localhost:${PORT}`);
