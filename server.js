...
function readDb(){ensureDb();try{const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8'));const merged={...initialDb(),...db,settings:{...defaultSettings,...(db.settings||{}),delivery:{...defaultSettings.delivery,...(db.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db.settings?.seo||{})}},promos:Array.isArray(db.promos)?db.promos:defaultPromos,audit:Array.isArray(db.audit)?db.audit:[]};const dom=String(merged.settings.siteDomain||'').toLowerCase();if(['barkamarket.uz','barakamarket.uz'].includes(dom))merged.settings.siteDomain='velora.uz';for(const k of ['title','description','keywords'])if(typeof merged.settings.seo?.[k]==='string')merged.settings.seo[k]=merged.settings.seo[k].replace(/BarkaMarket\.uz/gi,'VELORA.UZ').replace(/barkamarket/gi,'velora').replace(/barakamarket/gi,'velora');return merged;}catch{return initialDb();}}
