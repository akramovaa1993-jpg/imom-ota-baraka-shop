...
const defaultSettings={
 ...
 seo:{title:'IMOM OTA BARAKA — VELORA.UZ',description:'VELORA.UZ — Parkent tumani bo‘ylab bepul yetkazib beruvchi IMOM OTA BARAKA internet do‘koni.',keywords:'barkamarket, imom ota baraka, parkent, internet do‘kon, bepul yetkazib berish'}
};
...
function readDb(){ensureDb();try{const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8'));return {...initialDb(),...db,settings:{...defaultSettings,...(db.settings||{}),delivery:{...defaultSettings.delivery,...(db.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db.settings?.seo||{})}},promos:Array.isArray(db.promos)?db.promos:defaultPromos,audit:Array.isArray(db.audit)?db.audit:[]};}catch{return initialDb();}}
...
app.get('/api/admin/backup.json',requireAdmin,(req,res)=>{res.setHeader('Content-Disposition',`attachment; filename="barkamarket-backup-${new Date().toISOString().slice(0,10)}.json"`);res.json(readDb());});
...
app.get('/api/admin/reports.xls',requireAdmin,(req,res)=>{
 ...
 res.setHeader('Content-Type','application/vnd.ms-excel; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="barkamarket-report-${Date.now()}.xls"`);res.send('\ufeff'+html);
});
