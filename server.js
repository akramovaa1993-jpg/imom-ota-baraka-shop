...
const defaultSettings={
 siteName:'IMOM OTA BARAKA',siteDomain:'velora.uz',theme:'original',
 ...
 map:{eyebrow:'BEPUL YETKAZIB BERISH HUDUDI',title:'Parkent tumani xaritasi',text:'Buyurtmalar Parkent tumani bo‘ylab bepul yetkazib beriladi. Buyurtma vaqtida hududingizni tanlang va kerak bo‘lsa aniq GPS lokatsiyangizni yuboring.',embedUrl:'https://www.openstreetmap.org/export/embed.html?bbox=69.50%2C41.12%2C69.88%2C41.45&layer=mapnik&marker=41.2944%2C69.6764',openUrl:'https://www.openstreetmap.org/?mlat=41.2944&mlon=69.6764#map=11/41.2944/69.6764'},
 footer:{service1:'Parkent tumani — bepul',service2:'Naqd / karta / o‘tkazma',copyright:'© 2026 IMOM OTA BARAKA • VELORA.UZ'},
 ui:{logoSize:68},
 ...
 seo:{title:'IMOM OTA BARAKA — VELORA.UZ',description:'VELORA.UZ — Parkent tumani bo‘ylab bepul yetkazib beruvchi IMOM OTA BARAKA internet do‘koni.',keywords:'velora, imom ota baraka, parkent, internet do‘kon, bepul yetkazib berish'}
};
const defaultPromos=[{code:'BARAKA5',type:'percent',value:5,minTotal:150000,active:true,usageLimit:100,used:0,expires:''}];

function initialDb(){return {products:defaultProducts,categories:defaultCategories,settings:defaultSettings,logo:'',orders:[],promos:defaultPromos,audit:[]};}
function ensureDb(){fs.mkdirSync(DATA_DIR,{recursive:true});if(!fs.existsSync(DB_FILE))writeDb(initialDb());}
function readDb(){ensureDb();try{const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8'));const merged={...initialDb(),...db,settings:{...defaultSettings,...(db.settings||{}),delivery:{...defaultSettings.delivery,...(db.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db.settings?.seo||{})},company:{...defaultSettings.company,...(db.settings?.company||{})},map:{...defaultSettings.map,...(db.settings?.map||{})},footer:{...defaultSettings.footer,...(db.settings?.footer||{})},ui:{...defaultSettings.ui,...(db.settings?.ui||{})}},promos:Array.isArray(db.promos)?db.promos:defaultPromos,audit:Array.isArray(db.audit)?db.audit:[]};const dom=String(merged.settings.siteDomain||'').toLowerCase();if(['barkamarket.uz','barakamarket.uz'].includes(dom))merged.settings.siteDomain='velora.uz';for(const k of ['title','description','keywords'])if(typeof merged.settings.seo?.[k]==='string')merged.settings.seo[k]=merged.settings.seo[k].replace(/BarkaMarket\.uz/gi,'VELORA.UZ').replace(/barkamarket/gi,'velora').replace(/barakamarket/gi,'velora');return merged;}catch{return initialDb();}}
...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.0.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,dataFile:DB_FILE}));
...
app.get('/api/admin/reports.xls',requireAdmin,(req,res)=>{
 ...
 res.setHeader('Content-Type','application/vnd.ms-excel; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="velora-report-${Date.now()}.xls"`);res.send('\ufeff'+html);
});
...
app.listen(PORT,()=>{console.log(`IMOM OTA BARAKA v13 ORIGINAL / velora.uz: http://localhost:${PORT}`);console.log(`Telegram CHAT_ID: ${CHAT_ID?'configured':'MISSING'}`);console.log(`Telegram BOT_TOKEN: ${BOT_TOKEN?'configured':'MISSING'}`);console.log(`Online admin: /admin.html | DATA_DIR=${DATA_DIR}`);pollTelegram();});
