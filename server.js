...
function readDb(){ensureDb();try{const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8'));const merged={...initialDb(),...db,settings:{...defaultSettings,...(db.settings||{}),delivery:{...defaultSettings.delivery,...(db.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db.settings?.seo||{})},company:{...defaultSettings.company,...(db.settings?.company||{})},map:{...defaultSettings.map,...(db.settings?.map||{})},footer:{...defaultSettings.footer,...(db.settings?.footer||{})},ui:{...defaultSettings.ui,...(db.settings?.ui||{})}},promos:Array.isArray(db.promos)?db.promos:defaultPromos,audit:Array.isArray(db.audit)?db.audit:[]};const dom=String(merged.settings.siteDomain||'').toLowerCase();if(['velora.uz','barkamarket.uz','barakamarket.uz','imomotamarket.uz'].includes(dom))merged.settings.siteDomain='zarbuloq.uz';for(const k of ['title','description','keywords'])if(typeof merged.settings.seo?.[k]==='string')merged.settings.seo[k]=merged.settings.seo[k].replace(/VELORA\.UZ/gi,'ZARBULOQ.UZ').replace(/velora/gi,'zarbuloq').replace(/BarkaMarket\.uz/gi,'ZARBULOQ.UZ').replace(/barkamarket/gi,'zarbuloq').replace(/barakamarket/gi,'zarbuloq').replace(/imomotamarket/gi,'zarbuloq');return merged;}catch{return initialDb();}}
...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.10.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,dataFile:DB_FILE}));
...
app.listen(PORT,()=>{console.log(`IMOM OTA BARAKA v13.9 ZARBULOQ / zarbuloq.uz: http://localhost:${PORT}`);console.log(`Telegram CHAT_ID: ${CHAT_ID?'configured':'MISSING'}`);console.log(`Telegram BOT_TOKEN: ${BOT_TOKEN?'configured':'MISSING'}`);console.log(`Online admin: /admin.html | DATA_DIR=${DATA_DIR}`);pollTelegram();});
