...
function readDb(){ensureDb();try{const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8'));const merged={...initialDb(),...db,settings:{...defaultSettings,...(db.settings||{}),delivery:{...defaultSettings.delivery,...(db.settings?.delivery||{})},seo:{...defaultSettings.seo,...(db.settings?.seo||{})},company:{...defaultSettings.company,...(db.settings?.company||{})},map:{...defaultSettings.map,...(db.settings?.map||{})},footer:{...defaultSettings.footer,...(db.settings?.footer||{})},ui:{...defaultSettings.ui,...(db.settings?.ui||{})}},promos:Array.isArray(db.promos)?db.promos:defaultPromos,audit:Array.isArray(db.audit)?db.audit:[]};const dom=String(merged.settings.siteDomain||'').toLowerCase();if(['velora.uz','barkamarket.uz','barakamarket.uz','imomotamarket.uz'].includes(dom))merged.settings.siteDomain='zarbuloq.uz';if(merged.settings.map?.title==='Parkent tumani xaritasi')merged.settings.map.title='Yetkazib berish bepul hududlar';if(merged.settings.map?.text?.startsWith('Buyurtmalar Parkent tumani bo‘ylab bepul yetkazib beriladi'))merged.settings.map.text='Xaritada yashil rang va qizil chegara bilan ko‘rsatilgan Parkent tumani hududlarida yetkazib berish bepul. Buyurtma vaqtida manzilingizni tanlang yoki aniq GPS lokatsiyangizni yuboring.';merged.settings.map.openUrl='https://www.openstreetmap.org/relation/5745823';for(const k of ['title','description','keywords'])if(typeof merged.settings.seo?.[k]==='string')merged.settings.seo[k]=merged.settings.seo[k].replace(/VELORA\.UZ/gi,'ZARBULOQ.UZ').replace(/velora/gi,'zarbuloq').replace(/BarkaMarket\.uz/gi,'ZARBULOQ.UZ').replace(/barkamarket/gi,'zarbuloq').replace(/barakamarket/gi,'zarbuloq').replace(/imomotamarket/gi,'zarbuloq');return merged;}catch{return initialDb();}}
...
app.get('/api/status',(req,res)=>res.json({ok:true,version:'13.12.0',telegramConfigured:Boolean(BOT_TOKEN&&CHAT_ID),adminOnline:true,dataFile:DB_FILE}));
...
app.post('/api/product-requests',async(req,res)=>{
 try{
  ...
  db.productRequests=db.productRequests||[]; db.productRequests.unshift(item); db.productRequests=db.productRequests.slice(0,1000); audit(db,'mijoz','Mahsulot so‘rovi',requestId+' '+name); writeDb(db);
...
app.patch('/api/admin/product-requests/:id',requireAdmin,(req,res)=>{const db=readDb(),r=(db.productRequests||[]).find(x=>x.requestId===req.params.id);if(!r)return res.status(404).json({error:'So‘rov topilmadi'});const st=clean(req.body?.status,30);if(!['new','working','found','closed'].includes(st))return res.status(400).json({error:'Status noto‘g‘ri'});r.status=st;r.updatedAt=new Date().toISOString();audit(db,req.adminUser,'Mahsulot so‘rovi statusi',`${r.requestId}: ${st}`);writeDb(db);res.json({ok:true});});
...
app.listen(PORT,()=>{console.log(`IMOM OTA BARAKA v13.12 ZARBULOQ / zarbuloq.uz: http://localhost:${PORT}`);console.log(`Telegram CHAT_ID: ${CHAT_ID?'configured':'MISSING'}`);console.log(`Telegram BOT_TOKEN: ${BOT_TOKEN?'configured':'MISSING'}`);console.log(`Online admin: /admin.html | DATA_DIR=${DATA_DIR}`);pollTelegram();});
